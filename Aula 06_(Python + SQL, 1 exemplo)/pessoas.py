...
class Pessoas(Connection):
    ...
    def delete(self, id):
        try:
            ...
            sql_d = f"DELETE FROM pessoas WHERE id = {id}"
            self.execute(sql_d)
            self.commit()
    ...
    def update(self, id, *args):
        try:
            sql = f"UPDATE pessoas SET nome = %s WHERE id = {id}"
            self.execute(sql, args)
            self.commit()
...
if __name__ == "__main__":
    pessoas = Pessoas()
    #pessoas.insert("Luis")
    #pessoas.insert_csv("data.csv")
    #pessoas.delete(3)
    #pessoas.update(5, "Maria Antonio")
